        ...
        <>
            <div className='flex justify-between gap-1 bg-sky-600 md:px-2 rounded-lg h-10 md:h-12'>
                <div className='flex gap-2 md:justify-between items-center w-full'>
                  <div className='md:hidden ml-1 self-cneter'>
                    <button onClick={()=>onBackUser()} className='bg-white rounded-full px-2 py-1 self-center'>
                     <IoArrowBackSharp size={25} />
                    </button>
                  </div>
                  <div className='flex justify-between mr-2 gap-2'>
                    <div className='self-center'>
                      <img className='rounded-full w-6 h-6 md:w-10 md:h-10 cursor-pointer' src={selectedConversation?.profilepic} alt="" />
                    </div>
            ...
            <div className='flex-1 overflow-auto'>
                ...
                {!loading && messages?.length>0 && messages?.map((message)=>(
                  <div className='text-white' key={message?.id}  ref={lastMessageRef}>
                    <div className={`chat ${message.senderId===authUser._id ? 'chat-end':'chat-start '}`}> 
                        <div className='chat-image avatar'></div> 
                        <div className={`chat-bubble rounded-full ${message.senderId===authUser._id?'bg-sky-600':'bg-black'} text-white`}>
                          {message?.message}
                          </div>
                          <div className='chat-footer text-[10px] opactity-80 text-white'>
            ...
            <form onClick={handleSubmit} className='rounded-full text-black'>
                  <div className='w-full rounded-full flex items-center bg-white'>
                    <input value={sendData} required onChange={handleMessages} id='message' type='text' 
